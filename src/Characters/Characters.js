...
class Characters extends Component {
  ...
  componentDidUpdate() {
    if (
      this.state.selectedProgram !== this.props.selectedProgram ||
      this.state.update === true
    ) {
      axios
        .get(
          'http://localhost:8000/api/program/' +
  ...
  voteCharacterHandler = (name, description, votes, id, alt_text) => {
    ...
    axios
      .put('http://localhost:8000/api/characters/' + id, payload)
      .then(response => {
        ...
        return (
          <div key={character.id}>
            <Character
              ...
              voted={() =>
                this.voteCharacterHandler(
                  character.name,
                  character.description,
                  character.number_of_votes,
                  character.id,
                  character.image_alt_text
                )
              }
            />
          </div>
